...
export class FriendsController extends Controller {
    ...
    public async getAllFriendRequest(userId: string): Promise<frModel[]> {
        try {
            const users = await this.frManager
    ...
    public async CreateRequest(@Body() requestBody: frBody): Promise<any> {
        try {
            const frs = await this.frManager
                .getAllFriendRequest(requestBody.fromUser)
                .toPromise();
            frs.filter((fr: frModel) => {
                return fr.fromUser === requestBody.toUser;
            });

            if (frs.length !== 0) {
                throw "Friend request already open from User " +
                    requestBody.toUser +
                    " to User " +
                    requestBody.fromUser;
            }

            const frmodel: frModel = {
                dateOfRequest: String(new Date().getTime()),
                frId: "",
    ...
    public async acceptFriendRequest(frId: string): Promise<void> {
        try {
            console.log('Starting PUT friend request');
            //const frmodel:frModel={dateOfRequest:String(new Date().getTime()),frId:'',fromUser:requestBody.fromUser,toUser:requestBody.toUser} ;
            var data = await this.frManager
                .acceptFriendRequest(frId)
                .toPromise();
            console.log('Accepted Friend Request ', frId);
            this.setStatus(200);
        } catch (error) {
            console.error("Failed to accept friendRequest: ", error);
            this.setStatus(500);
            throw new CustomError(500, error);
        }
    }
