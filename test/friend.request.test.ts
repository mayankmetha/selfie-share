...
describe("Friend Requests API tests", () => {
    ...
    describe("Create Friend Request tests", () => {
        it("Should create a friend request", async () => {
            return new Promise(async resolve => {
                const response = await httpClient
                    .post("friendrequest", { fromUser: user1, toUser: user2 })
                    .toPromise();
                assert.isDefined(
                    response,
                ...
                assert.isDefined(
                    response.frId,
                    "Invalid friend request ID returned: " + response
                );
                frId = response.frId;
                resolve();
            });
        ...
        it("Should fail to create a duplicate friend request", async () => {
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    assert.fail(
                        "The request to create duplicate friend request succeeded!"
                    );
                    reject();
                } catch (error) {
                    console.log(
                        "Error creating duplicate FR: ",
                        error,
                        " which is expected"
                    );
        ...
        it("Should fail to create inverse friend request", async () => {
            try {
                const response = await httpClient
                    .post("friendrequest", { fromUser: user2, toUser: user1 })
                    .toPromise();
                console.log("Received response: ", response);
                assert.fail(
                    "The request to create inverse friend request succeeded!"
                );
            } catch (error) {
                console.log(
                    "Error creating inverse FR: ",
                    error,
                    " which is expected"
                );
            }
            ...
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    assert.equal(
                        JSON.parse(response).length,
                        0,
                        "Wrong number of friend requests returned"
                    );
                    resolve();
                } catch (e) {
                    reject(e);
    ...
    describe("Accept friend request API tests", () => {
        it("Should succeed in accepting friend request", async () => {
            return new Promise(async resolve => {
                await httpClient.put("friendrequest", frId, {}).toPromise();

                // Expect the friends API to return 1
                const friends: string = await httpClient
                    .get("users/" + user1 + "/friends")
                    .toPromise();
                assert.isDefined(friends, "Invalid response from server");
                assert.equal(
                    JSON.parse(friends).length,
                    1,
                    "Invalid number of friends returned!"
                );

                const friendsU2: string = await httpClient
                    .get("users/" + user2 + "/friends")
                    .toPromise();
                assert.isDefined(friendsU2, "Invalid response from server");
                assert.equal(
                    JSON.parse(friendsU2).length,
                    1,
                    "Invalid number of friends returned!"
                );

                // No friend requests open
                const frs: string = await httpClient
                    .get("users/" + user2 + "/friendrequest")
                    .toPromise();
                assert.isDefined(frs);
                assert.equal(
                    JSON.parse(frs).length,
                    0,
                    "Friend request still left open"
                );
                resolve();
            });
        });
        ...
        it("Should fail to accept already accepted request", async () => {
            try {
                await httpClient
                    .put("friendrequest", frId, { action: "accept" })
                    .toPromise();
                assert.fail("Accepted duplicate friend request");
            } catch (err) {
                return new Promise(async resolve => {
                    console.error(
                        "Failed to accept duplicate request: ",
                        err,
                        " which is expected"
                    );
                    resolve();
