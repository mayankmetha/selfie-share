...
describe("Shared Images API Tests", () => {
    ...
    before("Create Setup", () => {
        return new Promise(async resolve => {
            ...
            await httpClient.put("friendrequest", frId.frId, { action: "accept" }).toPromise();
    ...
    describe("Share images with user", () => {
        it("Should share an image with one user", () => {
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    const data = await httpClient.post("users/" + user1 + "/friends/" + user2 + "/images", [imageId]).toPromise();
                    console.log('\nSuccess: ', data);
                    resolve();
                } catch (error) {
                    console.log("\n\n--------------------\n\n");
                    ...
                    console.log("\n\n--------------------\n\n");
                    reject("One or more APIs or assertions failed");
                }
            });
        });
        ...
        it("Should fail to share already shared image", async () => {
            try {
                ...
                await httpClient.post("users/" + user1 + "/friends/" + user2 + "/images", [imageId]).toPromise();
        ...
        it("Should fail to share image that doesnt exist", async () => {
            try {
                await httpClient.post("users/" + user1 + "/friends/" + user2 + "/images", ["junkvalues"]).toPromise();
                assert.fail("Test failed - success returned where request should have failed");
            } catch (error) {
                if(String(error).indexOf('AssertionError') >=0 ) {
                    throw error;
    ...
    describe("Get Shared images with user", () => {
        ...
        it("Should return zero images shared with user", async () => {
            try {
                const images = await httpClient
                    .getSimpleFiltered("users/" + user1 + "/images", "sharedWith", user3)
                    .toPromise();
                assert.isDefined(images);
        ...
        it("Should return zero images shared by user", async () => {
            try {
                const images = await httpClient
                    .getSimpleFiltered("users/" + user1 + "/images", "sharedBy", user3)
                    .toPromise();
                assert.isDefined(images);
                assert.equal(JSON.parse(images).length, 0, "Wrong number of images found");
            } catch (error) {
    ...
    describe("Tests for unshared images", () => {
        ...
        it("Should fail to unshare an image that doesnt exist", async () => {
            try {
                await httpClient.delete("users/" + user1 + "/friends/" + user2 + "/images", "junk").toPromise();
                console.log("\n\n--------------------\n\n");
